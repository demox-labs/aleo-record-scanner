...
import { extractProgramName, getProgramType } from ".";

const helloProgram = `
  program hello_helloerasosa.aleo;
...
const nftProgram = `
    ...
    function add_minter:
        ...
        output r3 as leo_nft.aleo/add_minter.future;
    ...
    function update_toggle_settings:
        ...
        output r1 as leo_nft.aleo/update_toggle_settings.future;
    ...
    function open_mint:
        ...
        output r4 as leo_nft.aleo/open_mint.future;
    ...
    function mint:
        ...
        output r7 as leo_nft.aleo/mint.future;
    ...
    function claim_nft:
        ...
        output r6 as leo_nft.aleo/claim_nft.future;
    ...
    function authorize:
        ...
        output r2 as leo_nft.aleo/authorize.future;
...
export async function checkStandards() {
    const programType = await getProgramType(helloProgram);
    assert(programType === 'untyped', `programType should be untyped, but is ${programType}`);
    const programType2 = await getProgramType(nftProgram);
    assert(programType2 === 'nft', `programType should be nft, but is ${programType2}`);
    const programType3 = await getProgramType(ARC20_STANDARD);
    assert(programType3 === 'token', `programType should be token, but is ${programType3}`);
}

export async function checkExtractProgramName() {
    const programName = await extractProgramName(helloProgram);
    assert(programName === 'hello_helloerasosa.aleo', `programName should be hello_helloerasosa, but is ${programName}`);
    const programName2 = await extractProgramName(nftProgram);
    assert(programName2 === 'leo_nft.aleo', `programName should be leo_nft, but is ${programName2}`);
    const programName3 = await extractProgramName(ARC20_STANDARD);
    assert(programName3 === 'arc20_token.aleo', `programName should be arc20_token, but is ${programName3}`);
}
